...
if "last_result" in st.session_state:
    ...
    if "receiver_index" in st.session_state:
        st.session_state["receiver_index"] = min(
            st.session_state["receiver_index"], len(receivers) - 1
        )
    ...
    with col1:
        ...
        trace_idx = st.slider(
            "Receiver index",
            min_value=0,
            max_value=len(receivers) - 1,
            value=min(5, len(receivers) - 1),
            key="receiver_index",
        )
        fig_trace = go.Figure()
        fig_trace.add_trace(
